...
Note: The unofficial-polestar-api (gRPC) was evaluated but is PS4-only —
it uses a different OAuth client and VDMS endpoint that doesn't list PS2/PS3.
The extended fields (tyre pressure, exterior status, etc.) are in the frontend
types ready for when PS4 support is wired in.
"""
...
async def get_polestar() -> dict:
    ...
    try:
        ...
        _cache = {
            ...
            # Extended fields — only populated by unofficial-polestar-api (PS4).
            # Null/false here for PS2/PS3 via pypolestar.
            "charging_type": None,
            "voltage_volts": None,
            "washer_fluid_warning": None,
            "low_12v_battery": False,
            "any_light_failure": False,
            "tyre_warnings": {"fl": None, "fr": None, "rl": None, "rr": None},
            "tyre_pressure_kpa": {"fl": None, "fr": None, "rl": None, "rr": None},
            "is_locked": None,
            "any_door_open": False,
            "is_online": None,
        }
